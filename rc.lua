...
function terminal_callback()
   function callback(stdout, stderr, exitreason, exitcode)
   end
end
function detect_window_properties()
         ...
         naughty.notify(
            {
               ...
               title = "Detected Window Propeties",
               text = stdout
            }
         )
      end
...
--    {
--    }
-- function tagger:view_next()
--    awful.connect_for_each_screen(
--       function(op_screen)
--       for op_screen indo
--       end
--    end)
...
-- Prompt
   awful.key({ modkey }, "r", function () awful.spawn(tdrop_command) end,
   ...
   -- Menubar
   awful.key({ modkey }, "p", function() awful.spawn(rofi_drun_command) end,
   ...
   awful.key({ modkey,           }, "f",
      function (c)
         ...
         if c.fullscreen == true then
            c.border_width = defaults.border_width
         else
            c.border_width = 0 end
      end,
      {description = "toggle fullscreen", group = "client"}),
   awful.key({alt_l}, "F4", function (c) c:kill()                         end,
...
for i = 1, 9 do
   globalkeys = gears.table.join(
      ...
      awful.key({modkey, hyper_l}, "x", detect_window_properties)
   )
end
